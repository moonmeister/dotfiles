...
setup_sources_min() {
	...
	deb http://ppa.launchpad.net/git-core/ppa/ubuntu eoan main
	deb-src http://ppa.launchpad.net/git-core/ppa/ubuntu eoan main
	EOF
...
# sets up apt sources
# assumes you are going to use Ubuntu Eoan Ermine
setup_sources() {
	setup_sources_min;
	...
	deb http://security.ubuntu.com/ubuntu/ eoan-security universe main multiverse
	deb http://archive.ubuntu.com/ubuntu eoan-updates universe main multiverse
	EOF

	# yubico
	cat <<-EOF > /etc/apt/sources.list.d/yubico.list
	deb http://ppa.launchpad.net/yubico/stable/ubuntu eoan main
	deb-src http://ppa.launchpad.net/yubico/stable/ubuntu eoan main
	EOF
	...
	deb http://ppa.launchpad.net/linrunner/tlp/ubuntu eoan main 
	EOF


	# add the tlp apt-repo gpg key
	apt-key adv --keyserver hkp://p80.pool.sks-keyservers.net:80 --recv-keys 2042F03C5FABD0BA2CED40412B3F92F902D65EFF
	
	# Next-cloudclient
	cat <<-EOF > /etc/apt/sources.list.d/next-cloud-client.list
	deb http://ppa.launchpad.net/nextcloud-devs/client/ubuntu eoan main
	deb-src http://ppa.launchpad.net/nextcloud-devs/client/ubuntu eoan main
	EOF

	# add the next-cloud -client key
	apt-key adv --keyserver hkp://p80.pool.sks-keyservers.net:80 --recv-keys 1FCD77DD0DBEF5699AD2610160EE47FBAD3DD469

	# Add the Cloud SDK for Azure
	# echo "deb [arch=amd64] https://packages.microsoft.com/repos/azure-cli/ eoan main" > /etc/apt/sources.list.d/azure-cloud.list
...
install_wmapps() {
	local pkgs=( feh regolith-desktop i3status )

	#setup regolith PPA
	cat <<-EOF > /etc/apt/sources.list.d/tlp.list
	# Regolith i3 PPA 
	# https://regolith-linux.org/
	deb http://ppa.launchpad.net/kgilmer/regolith-stable/ubuntu eoan main
	EOF

	#Regolith PPA key
	apt-key adv --keyserver hkp://p80.pool.sks-keyservers.net:80 --recv-keys 	BDA0E4BA891B327BB00DF1E076458020C35556DC
